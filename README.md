...
| 3 | **📡 Browser pane visible to students (Cast)** | Web pages can't be drawn into the composite (browsers block iframe pixel capture for privacy — every web app has this limit). Fixed with a one-tap **📡 Cast** button in the browser bar: it switches the live stage to a **real screen/tab capture**, so students instantly see the actual browser — scrolling, videos, everything. Tap again (or stop the share) and the broadcast returns to the normal split-screen composite. On Chrome the picker is pre-set to "This tab" for the cleanest result. |
